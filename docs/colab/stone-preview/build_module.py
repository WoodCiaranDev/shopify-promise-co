import numpy as np, os, sys, itertools, json
...
_cache={}
CLIENT_COLOURS=None  # set by 19_apply_client_colours.py
def parts(metal, rim="medium", halo="none"):
    ...
    # Centre stones: the live picker's real stone images (no AI), scaled uniformly into the bezel
    wc_=np.load(f"weight_centre_{metal}.npy")
    ...
    H={}
    for k in K:
        tp=f"transplants_ps/{metal}_{k}.jpg"
    ...
    if CLIENT_COLOURS:
        # client's tuner settings (see 19_apply_client_colours.py): same OKLCH maths as the tuner page
        from oklab import adjust as ok_adjust
        wc=np.load(f"weight_centre_{metal}.npy"); wh=np.load(f"weight_halo_{metal}.npy")
        lm=json.load(open("tuner_base.json"))["metals"][metal]
        for k in K:
            st=CLIENT_COLOURS["stones"][k]
            C[k]=ok_adjust(C[k],wc,st["centre"],lm[k]["centre"]["lmed"]).astype(np.float32)
            H[k]=ok_adjust(H[k],wh,st["halo"],lm[k]["halo"]["lmed"]).astype(np.float32)
