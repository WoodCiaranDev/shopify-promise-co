...
SIZE, WANT = 1600, 4096
OUT = os.environ.get("AI_OUT", "slots_v2")
...
BASE = ("Edit this product photograph of a gold halo ring. {what} "
        ...
        "faceted transparent gemstones with realistic internal reflections. The centre stone must be razor sharp right out to its girdle: crisp, well-defined facet edges all the way to where it meets the gold bezel, a clean dark girdle line against the metal, no soft glow, haze or blur at the stone edge, and no cloudiness or inclusions - a clean, eye-clean lab-created stone.")
...
def call(prompt):
    body = {"model": MODEL, "prompt": prompt, "resolution": "4K", "aspect_ratio": "1:1",
...
STONES_RUN = [s for s in STONES if not os.environ.get("STONE_KEYS") or s[0] in os.environ["STONE_KEYS"].split(",")]
for k, desc in STONES_RUN:
...
ONLY_CENTRE = os.environ.get("ONLY_CENTRE") == "1"
if ONLY_CENTRE: sys.exit(f"\nspend ${sum(spend):.4f} over {len(spend)} calls")
